  ...
  <div>
    ...
    <div id="ejemplos">
      ...
      <div id="ejemplo2">
        ...
        <div
          v-for="(potentialGift, index) in secretFriendGiveaway"
          :key="index"
        >
          <ul>
    ...
    <hr />
    <div id="classExample">
      <div v-for="listItem in myWatchlist" :key="listItem">
        <h1>{{ listItem.title }}</h1>
        <h2>{{ listItem.year }}</h2>
        <ul>
          <li
            v-for="streamingPlatform in listItem.whereToWatch"
      ...
      <h1>Things on Netflix COMPUTED:</h1>
      <div v-for="(listItem, index) in watchOnNetflix" :key="index">
        <h1>{{ listItem.title }}</h1>
        <h2>{{ listItem.year }}</h2>
        <ul>
          <li
...
//Después deberíamos tener una variable computada watchOnNetflix que filtre el array de myWatchlist según si la key whereToWatch incluye la string "Netflix".En el template también deberíamos renderizar watchOnNetflix con un título para separar las dos listas.

const myWatchlist = reactive([
  {
    title: "Movie1",
    year: 2022,
    whereToWatch: ["Cinema", "HBO"],
  },
  {
    title: "TVShow",
    year: 2022,
    whereToWatch: ["HBO", "Netflix"],
  },
  {
    title: "TVShow2",
    year: 2022,
    whereToWatch: ["Amazon", "Netflix"],
  },
]);

const watchOnNetflix = computed(() => {
  return myWatchlist.filter((item) => item.whereToWatch.includes("Netflix"));
});
</script>
